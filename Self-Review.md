...
Не устану говорить, что техлид Лёша нуууу мёёёёёёёёёёд, особенно после встречи на корпоративе)
...
- 2.1.1 Итоги пентеста\
	После 2 пентестов получили 2 пачки уязвимостей, которые нужно закрыть до конца года пока успеваем - большая часть уязвимостей закрыта, принимал участие в закрытие уязвимостей, помогал DevOps подтвердить то что уязвимость устранена, оставшиеся уязвимости в основном распределены по Разработчикам.

- 2.1.2 Развитие FreeIPA:
	- Настроить интеграцию с базой 1С для автоматического отключения учеток сотрудников - пока ждем перенос базы 1С в наше облако, хочу закрыть эту задачку к следующему ревью, но перенос базы 1С не зависит от нас, увы задачу не сделал;
...
- 2.1.6 Джун в platforma-security\
	Попробовал себя в качестве ментора для нового сотрудника команды platforma-securityт, Артем быстро вкатился в работу из-за удачного подхода обучению и менторства и обучению, сейчас у него хорошо получается решать те задачки, которые планируем для него. Передаю свой опыт и знания накопленные за это время по части Security и инфраструктуры.
  	Дополнительно получилось быть ментором не только для направления Security, но и для DevOps, передаю свои знания по инфраструктуре, написанию Python скриптов, работы с HashiCorp Vault и протокола LDAP Илье Звереву, эти знания Илья успешно применяет в работе над задачками, также рассказывал Саше Харалгину как у нас устроена сетевая инфраструктура в облаках Servers.ru и Yandex Cloud, объяснял почему у нас есть некоторые "исторически сложившиеся" решения.
...
- 2.2.5 Матрица доступов\
	Существует полноценная матрица доступов, которая описывает какой доступ должен получать человек для своей должности/как он может получить повышенные привилегии для выполнения рабочих задач - пока что отказались от полноценной матрицы доступов и решили для экономии ресурсов сделать "примитивную" матрицу доступов, которая наглядно показывает стандартные права для сотрудников разных должностей, эта табличка используется в основе скрипта для автоматизации выдачи доступа в FreeIPA, что сейчас пилится совместно с Ильей Зверевым. Это один из шагов для экономии ресурсов DevOps и Security, позволит меньше отвлекаться на рутинные задачки и и автоматизировать процесс выхода нового сотрудника. 
...
- Прошлый раз я писал что хочу улучшить понимание нашей инфраструктуры - думаю я успешно прокачался в этом, т.к. передавал свои знания по инфре Илье Звереву/Саше Харалгину/Артему Коклянову в части сетевой доступности облаков/исторических моментов в нашей инфре.

- Думаю что стоит попробовать себя в планировании задач не только на спринт для себя и Артема, но и активно поучаствовать в целях на PI/годовых целях команды
  
- Если планируется найм в команду Security - поучаствовать в нем в качестве технического интервьюера или в командной части собеседования.
